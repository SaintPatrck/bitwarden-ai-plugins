...
Sourced from outside Bitwarden and **pinned to a reviewed commit**, following the pattern Anthropic uses for third-party entries in its own marketplace. Their files are not copied into this repo; each entry references an upstream repository or subdirectory at a fixed commit. They are not Bitwarden practice, so they sit outside the capability and bundle layers.
...
| [`superpowers`](https://github.com/obra/superpowers)                                                     | Jesse Vincent | Teaches Claude brainstorming, subagent driven development with built-in code review, systematic debugging, and red/green TDD. Also teaches Claude how to author and test new skills.                                                      |

`plugin-dev` and `skill-creator` resolve from `anthropics/claude-plugins-official` at `340e33aef211`; `superpowers` resolves from `obra/superpowers` at `b36e0829c6d0`. The pin is the point: a floating `ref` would let upstream change what your team installs without review. To move to a newer upstream commit, update the entry's `sha` and say what changed in the pull request.
