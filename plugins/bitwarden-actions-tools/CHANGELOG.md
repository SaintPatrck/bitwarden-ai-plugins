...
- Renamed from `bitwarden-devops-engineer`. Every skill here is GitHub Actions practice, and the plugin ships no agent, so the role name described neither its contents nor its shape.
