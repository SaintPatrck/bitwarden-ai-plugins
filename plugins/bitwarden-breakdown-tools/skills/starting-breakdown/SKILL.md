...
arguments: jira
allowed-tools: Skill(developing-breakdown-spec), Read, Edit, Glob, Bash(git status:*), Bash(cp templates/breakdown.md:*), Bash(mkdir:*), mcp__plugin_bitwarden-atlassian-tools_bitwarden-atlassian__get_issue, mcp__plugin_bitwarden-atlassian-tools_bitwarden-atlassian__get_issue_remote_links
---
...
1. **Verify the working directory.** This skill runs from inside the `bitwarden/tech-breakdowns` working copy, which is both where breakdowns are written and where the canonical templates live. `Glob` for `templates/breakdown.md`; if it is not found, stop and tell the user to launch Claude Code from inside the tech-breakdowns repo.
...
5. **Locate the template.** The canonical template is `templates/breakdown.md` in the tech-breakdowns working copy, resolved in step 1.
6. **Copy the template into the new folder as `breakdown.md`**: copy `templates/breakdown.md` to `<team>/<JIRA-KEY>-<short-slug>/breakdown.md`. Do not edit the template itself.
