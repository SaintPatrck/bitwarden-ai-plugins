---
name: auditing-workflow-conventions
description: >-
...
This skill covers only what `bwwl` cannot check. For anything the linter enforces, invoke `Skill(bitwarden-github-action-tools:bitwarden-workflow-linter-rules)` — that skill is the source of truth for all `bwwl` rules, including their triggers and fix procedures. Do not report a finding here that duplicates a linter rule.
