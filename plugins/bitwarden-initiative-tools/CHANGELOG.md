...
- Renamed from `bitwarden-shepherd`; the slug is now the role bundle.
- Gained `navigating-the-initiative-funnel` and `running-work-transitions` from `bitwarden-delivery-tools` and `contributing-to-technical-strategy` from `bitwarden-tech-lead`, so the whole Software Initiative Funnel lives in one plugin instead of three.
- Holds the shepherd and tech-lead agents.

## [1.0.1] - 2026-07-10

### Changed

- Updated all `architecting-solutions` cross-plugin references from `bitwarden-tech-lead` to `bitwarden-delivery-tools` following the skill's move in `bitwarden-tech-lead` 3.0.0 / `bitwarden-delivery-tools` 2.2.0.
