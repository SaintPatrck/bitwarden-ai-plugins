...
- Bring `Skill(bitwarden-architecture-tools:architecting-solutions)` into play when the trade-offs are inside one team's codebase — that skill carries the team-scope architectural judgment heuristics.
...
- Related: `Skill(shepherding-an-initiative)` for the umbrella playbook, `Skill(running-a-proof-of-concept)` for what Research feeds into, `Skill(bitwarden-architecture-tools:architecting-solutions)` for the team-scope architectural judgment heuristics to apply when options live inside one team's domain.
