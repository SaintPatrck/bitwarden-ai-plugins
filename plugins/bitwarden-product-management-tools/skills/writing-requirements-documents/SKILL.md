...
Dispatch `Skill(requirements-elicitation)` to extract requirements from the gathered sources:
...
`bitwarden-security-tools` and `bitwarden-atlassian-tools` are **optional** enhancers, not prerequisites:

- When **`bitwarden-security-tools`** is installed, invoke `Skill(bitwarden-security-tools:bitwarden-security-context)` for security vocabulary and P01–P06 principles, and `Skill(bitwarden-security-tools:threat-modeling)` for features requiring formal threat modeling.
- When **`bitwarden-atlassian-tools`** is installed, invoke `Skill(bitwarden-atlassian-tools:researching-jira-issues)` for deep-reading Jira issues with all linked issues, sub-tasks, and supporting Confluence documentation, and its MCP tools (`get_confluence_page`, `search_confluence`, `search_confluence_cql`) for standalone Confluence operations not tied to a Jira issue.

If either is unavailable, degrade gracefully:

- For security: document security considerations directly in the standard requirement sections.
- For Jira/Confluence: ask the user to provide content via alternative means (paste text, upload file).

## Related

Once a requirements document exists and implementation planning begins, hand off to `developing-breakdown-spec` in the `bitwarden/tech-breakdowns` repository for task decomposition — task list, dependencies, acceptance criteria, owners, grounded in real file/crate paths, resumable. This is a pointer to the next step, not a hard dependency; `bitwarden/tech-breakdowns` is a separate repository, not a marketplace plugin.
